...
if __name__ == '__main__':
    ...
    for i, (x, y, outs) in enumerate(iterate_batches(ds, batch_size=cfg.optim_args.batch_size,
        ...
        # outfile = open(outdir / 'res.jsonl', 'w')
                        ...
                        header = f'''\
batch_num, dt_data,dt,dt_total,loss_item,loss_bigram,loss_head,\
acc_tot,acc_end,acc_end_test,\
probe_score_start_acc, probe_score2_acc,probe_score_cond_acc,pred_attended_acc,repeat_frac,\
wk0_acc,wk0_64_acc,wk1_acc,wo1_acc,ff1_loss\n'''
                        file.write(header)
                    next_line  = \
                        f'''{i},{dt_data:.2f}, {dt:.2f}, {t - t0:.2f},{loss.item():.4f},{loss_bigram:.4f}, {loss_head:.4f}, \
{acc_tot:.4f},{acc_end:.4f},{acc_end_test:.4f} \
,{score_start_acc:.4f},{score2_acc:.4f},{score_cond_acc:.4f},{pred_attended_acc:.4f},{repeat_frac:.4f},\
{wk0_acc:.4f},{wk0_64_acc:.4f},{wk1_acc:.4f},{wo1_acc:.4f},{ff1_loss:.4f}\n'''
